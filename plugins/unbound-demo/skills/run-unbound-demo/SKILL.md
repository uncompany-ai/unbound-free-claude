---
name: run-unbound-demo
description: Demo tier (offline, fixture-backed) — Entry point and orchestrator for an Unbound working session. Use when the rep starts their daily event-processing ritual — reads run state, discovers events (calls + unanswered inbound emails) since the last run, presents the annotated slate, and walks the rep through one account or project at a time.
tier: all
---
# run-unbound-demo
...
> This is the **demo** build, and it may be installed alongside the standard Unbound pack. Resolve
> every skill reference in the prose above **within this pack**, never to a separately installed
> skill of the same name:
>
> - Bare `run-unbound` (as in "the run-unbound loop") means **this skill** — you are already in it.
> - `collect-tasks` means the bundled `resources/side-collect-tasks.md` in this same skill folder.
> - The eight numbered composition steps are the `resources/1-…8-write-crm.md` files bundled here.
>
> Never route to an installed `run-unbound`, `collect-tasks`, `connect-tools` or `setup-unbound`:
> those are backed by the live bindings and would attempt real connector calls. Everything this run
> needs is inside this folder, and every external read resolves to `resources/demo/fixtures/`.
