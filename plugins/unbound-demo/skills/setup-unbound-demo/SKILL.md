...
> This is the **demo** build, and it may be installed alongside the standard Unbound pack.
>
> **The readiness gate resolves `connect-tools` from `resources/connect-tools.md` bundled in this
> same skill folder — never from a separately installed skill of that name.** If a `connect-tools`
> skill is registered in this workspace, it belongs to the standard pack, is backed by the live
> bindings, and invoking it would attempt a real connector call. Open the bundled file instead; it is
> the only correct resolution of that step in this pack, and it is present by construction.
>
> The same rule governs every other bare skill reference in the prose above: `run-unbound` means
> `run-unbound-demo` from this pack, and `collect-tasks` means `collect-tasks-demo`. Every capability
> named here resolves through the demo `resources/tool-bindings.md`, against bundled fixture data.
