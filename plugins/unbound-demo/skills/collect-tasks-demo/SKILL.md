...
> `run-unbound` composition this skill's prose refers to — the loop it is a side entry beside, and
> whose single-threading it deliberately breaks — is **`run-unbound-demo`** from this same demo pack.
> Never resolve it to a separately installed `run-unbound`: that one is backed by the live bindings
> and would attempt real connector calls. This skill itself reads no external data; it works from the
> `accounts/` and `projects/` tree of the working directory, exactly as it does off-demo.
