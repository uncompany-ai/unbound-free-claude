---
name: connect-tools-demo
description: Demo tier (offline, fixture-backed) — Dual-entry environment readiness check. Derives the required capability set from runtime/tool-bindings.md's own binding tables, enumerates the live runtime's tool inventory, matches the two, proposes binding-row updates one at a time, and reports every gap with its concrete run-time cost. Invoked standalone by the rep ("check my connections", "am I ready to run", after adding or removing a connector or switching runtimes) and by setup-unbound at its readiness gate — NOT part of the run-unbound loop.
tier: all
---
# connect-tools-demo
...
## Skill references in this pack (demo tier)

> This is the **demo** build, and it may be installed alongside the standard Unbound pack. The
> `setup-unbound` this skill's prose names as its second entry — the readiness gate that invokes it —
> is **`setup-unbound-demo`** from this same demo pack, and that skill reaches this step through its
> own bundled `resources/connect-tools.md` copy rather than by routing to an installed skill. The
> `run-unbound` loop referenced here is likewise **`run-unbound-demo`**. Never resolve either name to
> a separately installed skill: those are backed by the live bindings.
>
> The bindings this skill inspects and maintains are the demo bindings bundled beside it as
> `resources/tool-bindings.md` — every capability there resolves to bundled fixture data, so a demo
> readiness report describes the fixture-backed runtime, not the presenter's live connectors.
